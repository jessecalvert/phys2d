...
* Last modified: November 27, 2017, 14:49
*/
...
LOAD_ENTIRE_TEXT_FILE_AND_NULL_TERMINATE(CRTLoadTextFileAndNullTerminate)
{
	char *Result = 0;
	...
	if(Error == 0)
	{
		umm FileSize = CRTGetFileSize(File);
		if(FileSize)
		{
			Result = PushArray(Region, FileSize + 1, char);
			umm BytesRead = fread(Result, FileSize, 1, File);
		}
	...
	{)MULTILINE",
					...
					++MemberIndex)
				{
					...
					string String = FormatString(Buffer, ArrayCount(Buffer),
						"\n        case %d: {DEBUG_VALUE_NAME(SimpleValue, \"%s\");} break;",
						Member->Value,
						Member->Name);
						...
						if(IsComplex)
						{
							...
							if(Member->IsPointer)
							{
								DebugValueMember = FormatString(Buffer, ArrayCount(Buffer),
									"\n        RecordDebugValue_(ValuePtr->%s, DEBUG_NAME(\"%s\"), Depth+1);\n    }", Member->Name, Member->Name, Member->Name);
							}
							else
							{
...
LoadSource(state *State, string Filename)
{
	...
	string SourcePath = FormatString(Buffer, ArrayCount(Buffer), "%s\\%s",
		State->CWD,
		Filename);
	char *SourceCString = Platform->LoadEntireTextFileAndNullTerminate(&State->Region, SourcePath.Text);
	if(SourceCString)
	{
		string Source = String(SourceCString);
...
s32 main(s32 ArgumentCount, char **Arguments)
{
	platform Platform_ = {};
	Platform = &Platform_;
	Platform->LoadEntireTextFileAndNullTerminate = CRTLoadTextFileAndNullTerminate;
	...
	State->CWD = String(Arguments[1]);
	string SourceFilename = String(Arguments[2]);
	string DestFilename = String(Arguments[3]);

	printf("Preprocessing %s ... ", Arguments[2]);
